...
class FootballCalendarEvent:
    ...
    def to_football_calendar_events(api_response: list[dict]):
        return [
            FootballCalendarEvent.to_football_calendar_event(x) for x in api_response
        ]
    ...
    def to_football_calendar_event(fixture: dict):
        ...
        venue = get_correct_venue_name(
            fixture.get("stadium_name"), fixture.get("stadium_city")
        )
        description = (
        ...
        return FootballCalendarEvent(
            match_id=fixture[
                "match_id"
            ],  # NOTE: want KeyError to be thrown if this doesn't exist
            summary=summary,
            location=venue,
...
class FootballCalendar:
    ...
    def to_calendar(self, url_path: str, home=False, away=False) -> Calendar:
        ...
        cal.add(
            "X-WR-RELCALID",
            f"{team_name_modified}{home_away_suffix}-{self.seasons}".replace(" ", ""),
        )
