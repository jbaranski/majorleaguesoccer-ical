...
def get_data(seasons: list[tuple[str, str]]):
    ...
    for season_id, season_year in seasons:
        response = requests.get(
            f"https://stats-api.mlssoccer.com/competitions/{LEAGUE}/seasons/{season_id}/standings"
        )
...
def main() -> None:
    ...
    for t_id, f in team_fixtures.items():
        ...
        team_file_name = (
            team_name.replace(".", "").replace(" ", "").replace("\n", "").lower()
        )
        ...
        logging.info(
            f"Calendar generated: num_fixtures={len(f)}, team={t_id}|{team_name}, seasons={season_years_str}, path={calendar_path}, home path={calendar_path_home}, away path={calendar_path_away}"
        )
    ...
    with open(mls_calendar_path, "wb") as cf:
        cf.write(
            FootballCalendar.to_football_calendar(
                "MLS",
                season_years_str,
    ...
    logging.info(
        f"Calendar generated: num_fixtures={len(all_fixtures)}, calendar=MLS, seasons={season_years_str}, path={mls_calendar_path}"
    )
